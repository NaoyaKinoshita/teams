...
Teams 会議の録画開始・停止を検知し、Azure に通知するアプリ。
...
└── functions/      # Azure Functions（録画停止通知の受け口）
```
...
タブ内に「Azure に連携する」「スキップ」ボタンを表示
    ├── 「Azure に連携する」クリック → 同意を記録
    └── 「スキップ」クリック → 通知しない
    ↓
録画が停止 → 同意済みの場合のみ Azure Functions へ Webhook 通知
```
...
| コンポーネント | 技術 |
|---|---|
| Bot サーバー | Python / aiohttp + Bot Framework SDK |
| 録画検知 | Microsoft Graph API 変更通知（`chats/{id}/messages`） |
| 会議チャット自動検知 | Graph API `/chats` 購読 |
| コンセント UI | Teams JS SDK + HTML タブ（サイドパネル） |
| Azure 通知 | Azure Functions HTTP トリガー（Webhook） |
| ローカル開発用トンネル | ngrok |
...
- [functions/README.md](functions/README.md) — Azure Functions のセットアップ手順
