...
会議サイドパネルのタブがポーリングで録画開始を検知
    ↓
コンテンツバブルとタブ内に「Azure に保存する」「スキップ」ボタンを表示
    ├── 「Azure に保存する」クリック → 同意を POST /api/consent に記録
    └── 「スキップ」クリック → 不同意を記録
    ↓
...
```
meeting-app/
├── app.py              # FastAPI サーバー・ルーティング
├── bot.py              # Bot Framework ハンドラー
├── graph_client.py     # Graph API 操作・録画状態管理
├── config.py           # 環境変数読み込み
├── templates/
│   ├── tab.html        # 会議サイドパネルタブ UI
│   └── notification.html # コンテンツバブルから開く通知 UI
├── manifest/
...
| GET | `/notification` | コンテンツバブルから開く通知 UI の HTML |
| POST | `/api/tab-context` | タブからの threadId 受け取り・チャット購読作成 |
| GET | `/api/recording-status` | 録画状態のポーリング（タブ用） |
| POST | `/api/consent` | Azure 保存の同意/スキップ受け取り |
...
| `targetedMeetingNotification` では `dialog.url.submit()` が使えない | ボタン押下後にパネル内でインライン表示に切り替え、手動クローズ |
